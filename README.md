...
### Installing from a private Git host

If `github.com` is a private / self-hosted Git server (e.g. a company GitLab),
Go's default module proxy (`proxy.golang.org`) and checksum DB (`sum.golang.org`)
cannot reach it, so `go get` will fail with a 404 / `unknown revision` error
unless you tell Go it is private:

```shell
# Treat this host as private: skip module proxy and checksum DB
go env -w GOPRIVATE=github.com/*
# Or export in your shell profile instead:
# export GOPRIVATE=github.com/*
```

If the server does not expose the `?go-get=1` meta endpoint over HTTPS,
either allow insecure module fetching:

```shell
go env -w GOINSECURE=github.com/*
```

…or force Go to use SSH (recommended when you already have an SSH key
...
After that, `go get github.com/gate/gateapi-go/v7@<tag>` will work.

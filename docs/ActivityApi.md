...
> GetMyActivityEntryResponse GetMyActivityEntry(ctx, )
...
func main() {
    ...
    result, _, err := client.ActivityApi.GetMyActivityEntry(ctx)
    if err != nil {
...
> ListActivitiesResponse ListActivities(ctx, optional)
...
func main() {
    ...
    result, _, err := client.ActivityApi.ListActivities(ctx, nil)
    if err != nil {
        if e, ok := err.(gateapi.GateAPIError); ok {
            fmt.Printf("gate api error: %s\n", e.Error())
        } else {
...
> ListActivityTypesResponse ListActivityTypes(ctx, )
...
func main() {
    ...
    result, _, err := client.ActivityApi.ListActivityTypes(ctx)
    if err != nil {
        if e, ok := err.(gateapi.GateAPIError); ok {
            fmt.Printf("gate api error: %s\n", e.Error())
        } else {

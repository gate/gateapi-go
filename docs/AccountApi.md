...
func main() {
    ...
    result, _, err := client.AccountApi.GetAccountMainKeys(ctx)
    if err != nil {
        if e, ok := err.(gateapi.GateAPIError); ok {
            fmt.Printf("gate api error: %s\n", e.Error())
        } else {
...
**optional** | **ListSTPGroupsOpts** | optional parameters | nil if no parameters

### Optional Parameters

Optional parameters are passed through a pointer to a ListSTPGroupsOpts struct
...
func main() {
    ...
    result, _, err := client.AccountApi.ListSTPGroups(ctx, nil)
    if err != nil {
        if e, ok := err.(gateapi.GateAPIError); ok {
            fmt.Printf("gate api error: %s\n", e.Error())
        } else {
...
func main() {
    ...
    stpGroup := gateapi.StpGroup{} // StpGroup - 
    
    result, _, err := client.AccountApi.CreateSTPGroup(ctx, stpGroup)
    if err != nil {
        if e, ok := err.(gateapi.GateAPIError); ok {
            fmt.Printf("gate api error: %s\n", e.Error())
        } else {
